...
# from langchain_community.vectorstores import Pinecone as LangChainPinecone  # Updated
...
# # Flask app setup
# app = Flask(__name__)
# CORS(app, resources={r"/get": {"origins": "http://localhost:3000"}})  # Allow only React frontend
...
def chat():
    try:
        ...
        msg = data.get("msg", "").strip()  # Extract message

        if not msg:
            return jsonify({"answer": "Please provide a valid question."}), 400

        print("User Input:", msg)
        response = rag_chain.invoke({"input": msg})  # Process input
