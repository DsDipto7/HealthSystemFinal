...
const DiseasePrediction = () => {
  ...
  const handleButtonClick = async () => {
    try {
      const formattedSymptoms = symptoms.split(',').map(s => s.trim()).filter(s => s);
      const payload = { symptoms: formattedSymptoms };

      // Generate curl command
      const curlCmd = `curl -X POST http://127.0.0.1:5000/api/predict -H "Content-Type: application/json" -d '${JSON.stringify(payload)}'`;
